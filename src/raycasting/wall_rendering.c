...
void	draw_textured_wall(t_mlx *mlx, int x, t_ray *ray, t_player *player)
{
	...
	tex_x = calculate_texture_x(mlx, ray, wall_x, tex_index);
...
void	draw_vertical_line(t_mlx *mlx, int x, t_ray *ray, t_map *map_data)
{
	...
	floor_color = (map_data->config.floor_color[0] << 16)
		| (map_data->config.floor_color[1] << 8)
		| map_data->config.floor_color[2];
	ceiling_color = (map_data->config.ceiling_color[0] << 16)
		| (map_data->config.ceiling_color[1] << 8)
